...
export const MyProductsCard = (props: MyProductCardProps) => {
	...
	const pushEditProduct = async (id: string) => {
		...
		await router.push({
			pathname: '/mypage',
			query: { category: 'addProduct', propertyId: id },
		});
	};
		...
		return (
			<Stack className="property-card-box">
				<Stack className="image-box" onClick={() => pushProductDetail(product?._id)}>
					<img src={`${process.env.NEXT_PUBLIC_APP_API_URL}/${product.productImages[0]}`} alt="" />
				</Stack>
