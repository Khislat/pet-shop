...
const MyProfile: NextPage = ({ initialValues, ...props }: any) => {
	...
	const [updateData, setUpdateData] = useState<MemberUpdate>(defaultValues);
	...
	/** LIFECYCLES **/
	useEffect(() => {
		// Page load bo'lganda localStorage dan token olish va user ma'lumotlarini yuklash
		const token = getJwtToken();
	...
	useEffect(() => {
		// user ma'lumotlari yuklangandan keyin updateData ni yangilash
	...
	const uploadImage = async (e: any) => {
		try {
			...
			return `${NEXT_PUBLIC_APP_API_URL}/${responseImage}`;
		} catch (err) {
	...
	const updatePropertyHandler = useCallback(async () => {
		try {
			// user va user._id mavjudligini tekshirish
			if (!user || !user._id) throw new Error(Messages.error2);
	...
	// user yuklanmagan yoki _id bo'sh bo'lsa loading ko'rsatish
	if (!user || !user._id) {
						...
						<Stack className="image-big-box">
							<Stack className="image-box">
								<img
									src={
										updateData?.memberImage
											? `${NEXT_PUBLIC_APP_API_URL}/${updateData?.memberImage}`
